  ...
  function handleStartStop(newTimerState = timerState) {
    if (!timer.timerOn) {
      ...
      const timerInterval = setInterval(() => {
        const sec = Math.floor((initialIntervalSecounds + (Date.now() - timeStamp) / 1000))
        setTimer({
          secounds: sec,
          timerOn: true,
          timerInterval,
    ...
    <div className="App">
      <div id="container">
        <div id="timer">
          ...
          <div id="timer-value" style={getColor(timer)}>{timerDisplay(timer)}</div>
          <div id="controls">
            <div type="button" className="control" onClick={handleStartStop}>{timer.timerOn ? 'stop' : 'start'}</div>
            <div className="control" onClick={handleReset}>reset</div>
          </div>
        </div>
        
        <div id="session">
          <p id="session-label">Session Length: {'\u00A0'}{sessionValue}</p>
          <Buttons handleClick={handleSessionChange}/>
        </div>

        <div id="break">
          <p id="break-label">Break Length: {'\u00A0'}{breakValue}</p>
